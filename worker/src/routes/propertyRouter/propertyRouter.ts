...
import { PropertyCreateSchema, PropertyCompleteSchema } from '../../schemas/apiSchemas/propertyApiSchemas';
...
property.post('/generate-description', zValidator('json', PropertyCreateSchema), async (c) => {
  try {
    const propertyData = c.req.valid('json');

    // Construct the prompt for GPT
    const prompt = `Generate a compelling property listing description for a ${propertyData.type} with the following details:
